...
if __name__ == '__main__':
    ...
    for i in hidden:
        for j in hidden2:
            ...
            scores.append([i,j, (sum(acc_score) / len(acc_score)), (sum(pre_score) / len(pre_score))])


    print(scores)
    scores_pd = pd.DataFrame(scores, columns=['1st Hidden # Neuron', '2nd Hidden # Neuron', 'Test Accuracy', 'Test Precision'])
    scores_pd.to_csv('Hidden Combination NN_v2.csv', index=False)
