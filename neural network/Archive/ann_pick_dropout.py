...
if __name__ == '__main__':
    ...
    dropout1 = np.arange(0, 0.25, 0.05)
    dropout2 = np.arange(0, 0.25, 0.05)
    dropout3 = np.arange(0, 0.25, 0.05)
    print(dropout1)
    # dropout = [0.01]
    pre_score = []
    acc_score = []
    ...
    for i in dropout1:
        for j in dropout2:
            for k in dropout3:
                ...
                scores.append([i,j,k, (sum(acc_score) / len(acc_score)), (sum(pre_score) / len(pre_score))])


    print(scores)
    scores_pd = pd.DataFrame(scores, columns=['dropout (first layer)', 'dropout (second layer)',
                                              'dropout (output layer)','Test Accuracy', 'Test Precision'])
    scores_pd.to_csv('Pick Dropout Combination NN_v2.csv', index=False)
